...
{
UI::UI(const FrameCallback& frameCallback, const KeyCallback& keyCallback, const std::wstring& title, int width, int height, bool vsync)
...
void UI::run()
{
    ...
    while (msg.message != WM_QUIT) {
        ...
        m_d3dSwapChain->Present(m_vsync ? 1 : 0, 0);
    }
...
void UI::terminate()
{
    const auto windowThreadId = GetWindowThreadProcessId(m_hWnd, NULL);
    PostThreadMessage(windowThreadId, WM_QUIT, 0, 0);
}
...
ComPtr<ID3D11Device> UI::getDevice() const { return m_d3dDevice; }

ComPtr<ID3D11DeviceContext> UI::getDeviceContext() const { return m_d3dDeviceContext; }

...
public:
    ...
    struct Segment {
        glm::vec3 a{};     // start (world)
        glm::vec3 b{};     // end   (world)
        float     radius{ 0.05f }; // meters, half-thickness of the “bone”
        float     alpha{ 1.0f };   // 0..1 (confidence → opacity)
        glm::vec3 worldOffset = glm::vec3(0.0f);
    };

    // app calls this once per frame after body-tracking
    void setSkeletonSegments(std::vector<Segment>&& segs);
...
private:
    ...
    // a tiny mesh + shader for bones (stretched cube works well)
    std::unique_ptr<VarjoExamples::Renderer::Mesh>   m_boneMesh;
    std::unique_ptr<VarjoExamples::Renderer::Shader> m_boneShader;

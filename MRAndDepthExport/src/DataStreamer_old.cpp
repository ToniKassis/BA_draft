...
{
DataStreamer_old::DataStreamer_old(varjo_Session* session)
    : m_session(session)
    , m_hmdPose(toVarjoMatrix(glm::mat4(0.0f)))
{
}

DataStreamer_old::~DataStreamer_old()
{
...
std::pair<varjo_StreamId, varjo_ChannelFlag> DataStreamer_old::getStreamingIdAndChannel(varjo_StreamType streamType, varjo_TextureFormat streamFormat) const
{
...
bool DataStreamer_old::isStreaming() const
{
    // Frame callback comes from different thread, lock streaming data
    std::lock_guard<std::recursive_mutex> streamLock(m_streamData.mutex);
...
bool DataStreamer_old::isStreaming(varjo_StreamType streamType, varjo_TextureFormat streamFormat) const
{
    return (getStreamingIdAndChannel(streamType, streamFormat).first != varjo_InvalidId);
}

varjo_TextureFormat DataStreamer_old::getFormat(varjo_StreamType streamType)
{
...
bool DataStreamer_old::isStreaming(varjo_StreamType streamType, varjo_TextureFormat streamFormat, varjo_ChannelFlag& outChannels) const
{
...
void DataStreamer_old::startDataStream(varjo_StreamType streamType, varjo_TextureFormat streamFormat, varjo_ChannelFlag channels)
{
...
void DataStreamer_old::stopDataStream(varjo_StreamType streamType, varjo_TextureFormat streamFormat)
{
    varjo_StreamId streamId = getStreamingIdAndChannel(streamType, streamFormat).first;
...
void DataStreamer_old::handleDelayedBuffers(bool ignore)
{
...
void DataStreamer_old::printStreamConfigs() const
{
    std::vector<varjo_StreamConfig> configs;
    configs.resize(varjo_GetDataStreamConfigCount(m_session));
    varjo_GetDataStreamConfigs(m_session, configs.data(), static_cast<int32_t>(configs.size()));
...
void DataStreamer_old::storeBuffer(varjo_StreamType type, varjo_StreamId streamId, varjo_ChannelIndex channelIdx, int64_t frameNumber, varjo_BufferId bufferId,
    varjo_BufferMetadata& buffer, void* cpuData, const std::string& baseName)
{
    ...
    if (buffer.type == varjo_BufferType_CPU) {
        ...
        if (type == varjo_StreamType_DistortedColor) {
            saveVSTBufferToArray(buffer,cpuData,channelIdx);
        }
        /*
...
void DataStreamer_old::handleBuffer(
...
void DataStreamer_old::dataStreamFrameCallback(const varjo_StreamFrame* frame, varjo_Session* session, void* userData)
{
    ...
    DataStreamer_old* streamer = reinterpret_cast<DataStreamer_old*>(userData);
    streamer->onDataStreamFrame(frame, session);
}

void DataStreamer_old::onDataStreamFrame(const varjo_StreamFrame* frame, varjo_Session* session)
{
    // Callback comes from different thread, lock streaming data
    std::lock_guard<std::recursive_mutex> streamLock(m_streamData.mutex);
    ...
    switch (frame->type) {
        case varjo_StreamType_DistortedColor: {
            ...
            //update acquireable vst image only after each eye frame data is written to m_latestVSTFrameData, to prevent a lookup where one image is updated and the other one not.
            if (!m_latestVSTFrame.data_l.empty() && !m_latestVSTFrame.data_r.empty()) {
                setLatestReleasedVSTFrame();
            }
...
varjo_StreamId DataStreamer_old::startStreaming(varjo_StreamType type, varjo_TextureFormat format, varjo_ChannelFlag channels)
{
...
bool DataStreamer_old::isDelayedBufferHandlingEnabled() const { return m_delayedBufferHandling; }

void DataStreamer_old::setDelayedBufferHandlingEnabled(bool enabled) { m_delayedBufferHandling = enabled; }

DataStreamer_old::ExposureAdjustments DataStreamer_old::getExposureAdjustments() const
{
    std::lock_guard<std::recursive_mutex> streamLock(m_streamData.mutex);
    return m_frameExposure;
}

varjo_Matrix DataStreamer_old::getHmdPose() const
{
    std::lock_guard<std::recursive_mutex> streamLock(m_streamData.mutex);
    return m_hmdPose;
}

bool DataStreamer_old::getCubemapFrame(CubemapFrame& frame) const
{
    std::lock_guard<std::recursive_mutex> streamLock(m_streamData.mutex);
...
void DataStreamer_old::saveVSTBufferToArray(const varjo_BufferMetadata& buffer, void* cpuData, varjo_ChannelIndex channelIdx) {
    constexpr int32_t components = 4;

    std::vector<uint8_t> data(buffer.height * buffer.height * components);
    ...
    case varjo_TextureFormat_RGBA16_FLOAT: {
        ...
        for (int32_t y = 0; y < buffer.height; y++) {
            ...
            for (int32_t x = 0; x < buffer.width * components; x += components) {
                ...
                // Read value, gamma correct, alpha blend to background color, write to one dimensional data array
                for (int32_t c = 0; c < 3; c++) {
                    float value = powf(DirectX::PackedVector::XMConvertHalfToFloat(halfSrc[x + c]), gamma);
                    value = value * alpha + rgbBackground[c] * (1.0f - alpha);
                    data[y * buffer.width * components + x + c] = static_cast<uint8_t>(std::max(0.0f, std::min(255.0f, 255.0f * value)));
                }

                // Write alpha
                data[y * buffer.width * components + x + 3] = 255;
                // line[x + 3] = static_cast<uint8_t>(max(0, min(255, 255.0 * alpha)));
            }
        }
    } break;
    ...
    case varjo_TextureFormat_YUV422: {
        ...
        for (int32_t y = 0; y < buffer.height; y++) {
            ...
            for (int32_t x = 0; x < buffer.width; x++) {
                ...
                // Write RGBA to one dimensional data array
                data[y * buffer.width * components + lineOffs + 0] = std::max(std::min(R, 255), 0);
                data[y * buffer.width * components + lineOffs + 1] = std::max(std::min(G, 255), 0);
                data[y * buffer.width * components + lineOffs + 2] = std::max(std::min(B, 255), 0);
                data[y * buffer.width * components + lineOffs + 3] = 255;
                lineOffs += components;
            }
        }
    } break;
    ...
    if (channelIdx == varjo_ChannelIndex_Left) {
        if (m_latestVSTFrame.data_l.size() != data.size()) {
            m_latestVSTFrame.data_l.resize(data.size());
        }
        memcpy(m_latestVSTFrame.data_l.data(), data.data(), data.size() * sizeof(uint8_t));
        m_latestVSTFrame.metadata_l = buffer;
    }
    else if (varjo_ChannelIndex_Right) {
        if (m_latestVSTFrame.data_r.size() != data.size()) {
            m_latestVSTFrame.data_r.resize(data.size());
        }
        memcpy(m_latestVSTFrame.data_r.data(), data.data(), data.size() * sizeof(uint8_t));
        m_latestVSTFrame.metadata_r = buffer;
    }
}

void DataStreamer_old::setLatestReleasedVSTFrame()
{
    if (!m_latestVSTFrame.data_l.empty()) {
        //copy left frame data
        if (m_latestReleasedVSTFrame.data_l.size() != m_latestVSTFrame.data_l.size()) {
            m_latestReleasedVSTFrame.data_l.resize(m_latestVSTFrame.data_l.size());
        }
        memcpy(m_latestReleasedVSTFrame.data_l.data(), m_latestVSTFrame.data_l.data(), m_latestVSTFrame.data_l.size() * sizeof(uint8_t));
        m_latestReleasedVSTFrame.metadata_l = m_latestVSTFrame.metadata_l;
    }
    
    if (!m_latestVSTFrame.data_r.empty()) {
        //copy right frame data
        if (m_latestReleasedVSTFrame.data_r.size() != m_latestVSTFrame.data_r.size()) {
            m_latestReleasedVSTFrame.data_r.resize(m_latestVSTFrame.data_r.size());
        }
        memcpy(m_latestReleasedVSTFrame.data_r.data(), m_latestVSTFrame.data_r.data(), m_latestVSTFrame.data_r.size() * sizeof(uint8_t));
        m_latestReleasedVSTFrame.metadata_r = m_latestVSTFrame.metadata_r;
    }
}

bool DataStreamer_old::getVSTFrame(VSTFrame& frame) const
{
     std::lock_guard<std::recursive_mutex> streamLock(m_streamData.mutex);

    // Check if we have a VST frame available.
    if (m_latestReleasedVSTFrame.data_l.empty() || m_latestReleasedVSTFrame.data_r.empty()) {
        return false;
    }

    // Create a copy of the VST data so it can be used safely without worrying about synchronization.
    frame = m_latestReleasedVSTFrame;
    return true;
}

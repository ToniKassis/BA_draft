...
public:
    ...
    struct VSTFrame {
        varjo_BufferMetadata metadata_l;
        std::vector<uint8_t> data_l;
        varjo_BufferMetadata metadata_r;
        std::vector<uint8_t> data_r;
    };

    //! Construct data streamer
    DataStreamer_old(varjo_Session* session);

    //! Destruct data streamer. Cleans up running data streams.
    ~DataStreamer_old();

    // Disable copy, move and assign
    DataStreamer_old(const DataStreamer_old& other) = delete;
    DataStreamer_old(const DataStreamer_old&& other) = delete;
    DataStreamer_old& operator=(const DataStreamer_old& other) = delete;
    DataStreamer_old& operator=(const DataStreamer_old&& other) = delete;
    ...
    bool getVSTFrame(VSTFrame& frame) const;
private:
    ...
    void saveVSTBufferToArray(const varjo_BufferMetadata& buffer, void* cpuData, varjo_ChannelIndex channelIdx);

    void setLatestReleasedVSTFrame();
private:
    ...
    VSTFrame m_latestReleasedVSTFrame;                 //!< Latest released VST Camera Frame for both eyes (uint8_t rgba)
    VSTFrame m_latestVSTFrame;                         //!< Latest VST Camera Frame for both eyes (uint8_t rgba)
